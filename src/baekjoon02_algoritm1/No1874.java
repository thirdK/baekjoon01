...
//2 초	128 MB	81995	29746	21033	35.915%
//문제
...
public class No1874 {
	public static void main(String[] args) throws IOException{
        ...
        int[] order = new int[len]; //출력해야할 순서를 기억할 배열
        ...
        int idx=0;//배열의 idx
		for(int i=1; i<=len; i++){ //1부터 len까지의 수를 사용하므로
		    stack.push(i);
		    sb.append("+\n");
			while(!stack.isEmpty() && stack.peek() == order[idx]){	//공백체크 안하면 peek()에서 오류
		        stack.pop();
		        sb.append("-\n");
		        idx++;
		    }
        ...
        if(!stack.isEmpty()) System.out.println("NO"); //스택에 data가 남아있다면 불가능

...
//각 숫자는 숫자+1 초의 시간이 걸린다.
//알파벳은 순서대로 숫자에 3개씩 등록되어 있는데 7과 9는 4개이다.
...
public class No5622 {
	public static void main(String[] args) throws IOException{
        ...
        int num = 2;    //2부터 알파벳이 존재함
        for(int i=0; i<alpha.length; i++){
            int numCase = 3; //알파벳이 3개씩 같은숫자에 속함
            if(num==7 || num==9) numCase=4; //7과 9는 4개
            for(int j=0; j<numCase; j++){
                alpha[i++] = num;
            }
            num++;
            i--; //바깥 for에서 무의미하게 i++되므로 여기서 감소
        }
            
        String str = br.readLine();        
        
        for(byte b : str.getBytes()){
            sec += alpha[b-'A']+1; //각 숫자는 숫자+1 초의 시간이 걸린다.
        }

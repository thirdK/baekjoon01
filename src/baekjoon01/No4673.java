...
public class No4673 {
	public static void main(String[] args) {
		...
		boolean[] isSelf = new boolean[10001]; //배열로 처리하려고함 (다른방법이 생각나지 않음)
		int result;
		
		
		for(int i=1; i<10001; i++) {
			result = d(i);	//d()메소드 호출하여 i의 결과를 받아옴
			if(result<=10000) { //10000 넘는 수는 제외하고
				isSelf[result] = true;//해당 숫자의 index에 true;
			}
		}
		
		for(int i=1; i<isSelf.length; i++) {
			if(!isSelf[i]) {//false인 index만 저장함
				sb.append(i).append("\n");
			}
		}
		System.out.print(sb);
	...
	public static int d(int n) {//정수 받아서 n+각자리수 를 구함
		int sum=n;
		while(n!=0) {
			sum += n%10;
			n/=10;
